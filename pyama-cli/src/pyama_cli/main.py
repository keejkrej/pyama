"""Command-line helpers for pyama-cli."""
...
def _prompt_microscopy_path() -> Path:
    """Prompt until the user provides a microscopy file path that exists."""
    while True:
...
def workflow() -> None:
    ...
    microscopy_path = _prompt_microscopy_path()
    ...
    try:
        with Progress(
            ...
            def _on_progress(
                event: WorkflowProgressEvent | WorkflowStatusEvent,
            ) -> None:
                if isinstance(event, WorkflowStatusEvent):
                    progress.update(
                        ...
                        description=(
                            f"{event.message} "
                            f"({event.completed_fovs}/{event.total_fovs} FOVs)"
                        ),
                    )
                    return

                total = max(1, event.frame_total)
                progress.update(
                    detail_task,
                    total=total,
                    completed=min(event.frame_index, total),
                    description=(
                        f"worker-{event.worker_id} {event.step} "
                        f"fov-{event.fov}: {event.message}"
                    ),
                )
...
def merge() -> None:
    ...
    processing_results_dir = _prompt_directory(
        "Enter the processing results folder",
        default=Path.cwd(),
        must_exist=True,
    )
    processing_results_path = processing_results_dir / "processing_results.yaml"
    ...
    try:
        message = run_core_merge(samples, processing_results_dir)
    except Exception as exc:  # pragma: no cover - runtime path
...
def statistics() -> None:
    ...
    folder_path = _prompt_directory(
        "Enter the merge_output folder",
        default=Path.cwd(),
        must_exist=True,
    )
    ...
    if mode == "onset_shifted_relu":
        fit_window_hours = float(
            typer.prompt("Onset fit window in hours", default=4.0)
        )

    area_filter_size = 10
    if normalize_by_area:
        area_filter_size = int(
            typer.prompt("Area filter size", default=10)
        )
