...
- **Always runs** if PC channel is configured, even with empty features
- If PC channel has no features, still outputs base fields: cell, frame, position, bbox
- FL channels: Processes configured features if available (intensity, particle count, etc.)
- Creates empty CSV if no channels configured at all
- Base fields are essential tracking data and don't require any explicit features
"""
...
class ExtractionService(BaseProcessingService):
    ...
    def process_fov(
        ...
        if traces_path.exists():
            logger.info("FOV %d: Traces CSV already exists, skipping extraction", fov)
            return
        ...
        if pc_channel is not None and pc_features:
            # Only include PC channel if it has configured features
