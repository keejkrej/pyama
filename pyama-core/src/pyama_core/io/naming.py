...
- seg: Binary segmentation mask (.npy)
- seg_labeled: Tracked cell labels (.npy)
...
def pc_stack(output_dir: Path, base_name: str, fov: int, channel: int) -> Path:
    ...
    return (
        ...
        / f"{base_name}_fov_{fov:03d}_pc_ch_{channel}.npy"
    )
...
def fl_stack(output_dir: Path, base_name: str, fov: int, channel: int) -> Path:
    ...
    return (
        fov_dir(output_dir, fov)
        / f"{base_name}_fov_{fov:03d}_fl_ch_{channel}.npy"
    )


def seg_mask(output_dir: Path, base_name: str, fov: int, channel: int) -> Path:
    """Get binary segmentation mask path.
    ...
    Returns:
        Path: {fov_dir}/{base_name}_fov_{fov:03d}_seg_ch_{channel}.npy
    """
    return (
        fov_dir(output_dir, fov)
        / f"{base_name}_fov_{fov:03d}_seg_ch_{channel}.npy"
    )


def seg_labeled(output_dir: Path, base_name: str, fov: int, channel: int) -> Path:
    """Get tracked/labeled segmentation path.
...
def discover_seg_labeled(output_dir: Path, fov: int) -> Path | None:
    """Discover labeled segmentation file for a FOV.
    ...
    Returns:
        Path to seg_labeled file if found, None otherwise
    """
    fov_path = fov_dir(output_dir, fov)
    if not fov_path.exists():
        return None
    matches = list(fov_path.glob("*_seg_labeled_ch_*.npy"))
