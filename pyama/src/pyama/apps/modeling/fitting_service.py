...
from pyama.apps.modeling.fitting import fit_trace_data
from pyama.apps.modeling.models import get_model
from pyama.io.csv.analysis import load_analysis_csv
from pyama.types.modeling import FitParam, FitParams, FixedParam, FixedParams
...
class FittingService:
    ...
    def __init__(self, progress_reporter: Callable[[dict], None] | None = None) -> None:
        """
        Args:
    ...
    def fit_csv_file(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                "Failed to save fitted results for %s: %s", saved_csv_path, exc
            )
            saved_csv_path = None
    ...
    def _build_fixed_params(
        ...
        for param_name, param in model.DEFAULT_FIXED.items():
            value = (
                model_params.get(param_name, param.value)
                if model_params
                else param.value
            )
