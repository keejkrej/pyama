    ...
    def process_fov(
        ...
        try:
            ...
            if isinstance(fl_background_entries, list):
                fl_background_map = {
                    int(id): Path(p) for id, p in fl_background_entries
                }
            ...
            for ch in sorted(channels_to_process):
                ...
                if cancel_event and cancel_event.is_set():
                    logger.info(
                        "FOV %d: Extraction cancelled at fluorescence channel %s",
                        fov,
                        ch,
                    )
                    return
                ...
                if fl_raw_path is None or not fl_raw_path.exists():
                    logger.warning(
                        "FOV %d: Raw fluorescence channel %s not found, skipping",
                        fov,
                        ch,
                    )
                    continue
                ...
                try:
                    ...
                    if fl_background_data is not None:
                        background_for_extraction = fl_background_data.astype(
                            np.float32
                        )
                        logger.info(
                        ...
                        background_for_extraction = np.zeros_like(
                            fl_raw_data, dtype=np.float32
                        )
                        logger.info(
                            "FOV %d: Extracting fluorescence features (%s) from channel %s",
                            fov,
