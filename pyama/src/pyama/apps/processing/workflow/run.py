...
def run_single_worker(
    ...
    try:
        from pyama.apps.processing.workflow.services import (
        ...
        def _service_reporter(payload: dict) -> None:
            if progress_reporter is None:
                return
            event = dict(payload)
            event.setdefault("event", "frame")
            event["worker_id"] = worker_id
            progress_reporter(event)

        segmentation.set_progress_reporter(_service_reporter)
        background_estimation.set_progress_reporter(_service_reporter)
        tracking.set_progress_reporter(_service_reporter)
        trace_extraction.set_progress_reporter(_service_reporter)
        ...
        if cancel_event and cancel_event.is_set():
            ...
            return (
                ...
                2,
                len(fovs) - 2,
                "Cancelled after background estimation",
                context,
            )
...
def run_complete_workflow(
    ...
    from pyama.apps.processing.workflow.services import CopyingService
    ...
    if progress_reporter is not None:
        copy_service.set_progress_reporter(
            lambda payload: progress_reporter(
                {
                    **payload,
    ...
    try:
        ...
        logger.info(
            "Processing selected FOVs in parallel with %d workers", worker_count
        )
        ...
        with ThreadPoolExecutor(max_workers=worker_count) as executor:
            futures = {
                ...
                for worker_id, fov_range in enumerate(worker_ranges)
                if fov_range
            }
            ...
            for future in as_completed(futures):
                ...
                if progress_reporter is not None:
                    progress_reporter(
                        {
                            "event": "status",
                            "completed_fovs": completed_fovs,
                            "total_fovs": total_fovs,
                            "progress_percent": progress,
                            "message": "Processing FOVs",
                        }
                    )
