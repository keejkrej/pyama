...
def _coerce_channel_name_sequence(value: Any) -> list[str]:
...
def _coerce_event_records(value: Any) -> list[dict[str, Any] | object]:
...
class NikonImage:
        ...
        try:
            ...
            if metadata and hasattr(metadata, "channels"):
                return _normalize_channel_names(
                    _coerce_channel_name_sequence(getattr(metadata, "channels")),
                    self._dims.C,
                )
        except Exception:
    ...
    def _load_timepoints(self) -> list[float]:
        try:
            records = _coerce_event_records(self._reader.events())
            timepoints: list[float] = []
            for index, event in enumerate(records):
                if isinstance(event, dict):
                    event_dict = cast(dict[str, Any], event)
                    value = event_dict.get("Time", index)
                else:
                    value = index
...
def load_microscopy_file(file_path: Path) -> tuple[MicroscopyImage, MicroscopyMetadata]:
    ...
    try:
        ...
        metadata = MicroscopyMetadata(
            ...
            channel_names=tuple(reader.channel_names),
            dtype=reader.dtype,
