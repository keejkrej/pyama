"""Modeling tab composed of data, fitting quality, and results panels."""
...
from pyama_gui.modeling.data import DataPanel
from pyama_gui.modeling.parameter import ParameterPanel
from pyama_gui.modeling.quality import QualityPanel
...
class ModelingTab(QWidget):
    """Embeddable modeling page comprising data, fitting quality, and parameter analysis panels.

    This tab orchestrates the interactions between the panels, managing signal
    routing and status updates for the modeling workflow. It provides a
    ...
    def _on_fitting_started(self) -> None:
        ...
        """
        logger.info("Modeling fitting started from Modeling tab")
        self.processing_started.emit()
        if self._status_manager:
            self._status_manager.show_message("Fitting models...")
    ...
    def _on_fitting_finished(self, success: bool, message: str) -> None:
        ...
        """
        logger.info(
            "Modeling fitting finished (success=%s): %s",
            success,
    ...
    def _on_fitting_completed(self, results) -> None:
        ...
        """
        logger.info("Modeling fitting completed with results payload")
        self.processing_finished.emit()
        if self._status_manager:
    ...
    def _on_data_loading_started(self) -> None:
        ...
        """
        logger.info("Modeling data loading started")
        if self._status_manager:
            self._status_manager.show_message("Loading modeling data...")
    ...
    def _on_data_loading_finished(self, success: bool, message: str) -> None:
        ...
        """
        logger.info(
            "Modeling data loading finished (success=%s): %s",
            success,
            message or "No message",
        )
        if self._status_manager:
    ...
    def _on_plot_saved(self, filename: str, directory: str) -> None:
        ...
        """
        logger.info("Modeling plot saved: %s (dir=%s)", filename, directory)
        if self._status_manager:
