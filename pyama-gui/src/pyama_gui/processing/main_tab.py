...
from pyama_gui.processing.input import InputPanel
from pyama_gui.processing.merge import MergePanel
from pyama_gui.processing.output import OutputPanel
        ...
        try:
            ...
            def _on_progress(event: object) -> None:
                if isinstance(event, WorkflowStatusEvent):
                    self.progress.emit(event.progress_percent, event.message)
...
class ProcessingTab(QWidget):
    ...
    def _on_microscopy_finished(
        ...
        if success and metadata:
            ...
            filename = (
                self._microscopy_path.name
                if self._microscopy_path
                else "microscopy file"
            )
    ...
    def _start_workflow(self) -> None:
        ...
        worker.progress.connect(self._on_workflow_progress)
        ...
        self._output_panel.set_progress_value(0)
    ...
    def _validate_parameters(self) -> bool:
        ...
        effective_fov_end = (
            n_fovs - 1
            if self._output_panel.fov_end == -1
            else self._output_panel.fov_end
        )
    ...
    def _on_workflow_progress(self, percent: int, message: str) -> None:
        self._output_panel.set_progress_value(percent)
        if self._status_manager and message:
            self._status_manager.show_message(f"{message} ({percent}%)")
