...
from pyama.tasks import read_samples_yaml, run_merge
from pyama_gui.constants import DEFAULT_DIR
from pyama_gui.types.processing import MergeRequest
...
class MergePanel(QWidget):
    ...
    def _on_merge_requested(self) -> None:
        ...
        try:
            samples = self.current_samples()
            processing_results_dir_text = (
                self._processing_results_dir_edit.text().strip()
            )
