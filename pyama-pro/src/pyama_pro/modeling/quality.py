"""Fitting quality inspection panel for the Modeling tab.
...
from pyama_core.modeling.fitting import analyze_fitting_quality
from pyama_core.modeling.models import get_model
from pyama_core.types.modeling import FitParam, FitParams
...
class QualityPanel(QWidget):
    ...
    # SIGNALS
    # ------------------------------------------------------------------------
    cell_visualized = Signal(
        object
    )  # Emitted when a cell is visualized ((fov, cell) tuple)
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self._fov_groups: dict[
            int, list[tuple[int, int]]
        ] = {}  # FOV -> list of (fov, cell)
    ...
    def set_results(self, df: pd.DataFrame) -> None:
        ...
        if "fov" in df.columns and "cell" in df.columns:
            self._cell_ids = [
                (int(row["fov"]), int(row["cell"])) for _, row in df.iterrows()
            ]
            ...
            logger.warning(
                "Results DataFrame missing fov/cell columns (columns=%s)",
                list(df.columns),
            )
            self._cell_ids = []
            self._fov_groups = {}
        ...
        for fov, cell in trace_ids:
            ...
            if self._results_df is not None:
                result_row = self._results_df[
                    (self._results_df["fov"] == fov)
                    & (self._results_df["cell"] == cell)
                ]
