...
This module provides the DataPanel widget for the Modeling tab, which handles:
...
from pyama_core.modeling.fitting_service import FittingService
from pyama_core.modeling.models import get_model, list_models
...
class DataPanel(QWidget):
    ...
    def _initialize_state(self) -> None:
        ...
        # Data state for the loaded modeling dataset
    ...
    def _on_load_fitted_results_clicked(self) -> None:
        ...
        """
        logger.debug("UI Click: Load fitted results button (start_dir=%s)", DEFAULT_DIR)
        file_path, _ = QFileDialog.getOpenFileName(
            self,
    ...
    def _on_start_clicked(self) -> None:
        ...
        """
        logger.debug(
            "UI Click: Start fitting button (csv_loaded=%s, model=%s)",
    ...
    def _collect_model_bounds(self) -> dict:
        ...
        return {
            ...
            if "min" in fields
            and "max" in fields
            and fields["min"] is not None
            and fields["max"] is not None
        }
    ...
    def _on_worker_finished(self, success: bool, message: str) -> None:
        ...
        if success:
            logger.info(
                "Modeling fitting completed (model=%s): %s",
                self._model_type,
                message,
            )
            ...
            if self._worker and hasattr(self._worker, "_processed_results"):
                for filename, results_df in self._worker._processed_results:
                    logger.info(
                        "Processed modeling file %s (%d rows)",
                        filename,
                        len(results_df),
                    )
...
class ModelingFittingWorker(QObject):
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the modeling worker.
    ...
    def process_data(self) -> None:
        ...
        try:
            ...
            logger.exception("Unexpected modeling worker failure")
